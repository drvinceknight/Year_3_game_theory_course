...
\begin{tikzpicture}[grow=right, sloped]
    ...
    \node[player]{a: Bob}
        ...
        child {node[player] {c: Celine}
                child {node[end] {} node[right] {$(0,0)$} edge from parent node[below] {Comedy}}
                child {node[end] {} node[right] {$(2,3)$} edge from parent node[below] {Sports}} edge from parent node[below] {Sports}};
    \node[player] at (0,5) {d: Bob}
        child{node[end] {} node[right] {$(4,2)$} edge from parent node[below] {Popcorn}}
        child{node[end] {} node[right] {$(7,3)$} edge from parent node[below] {Hotdog}};
        \draw [red,very thick] (-1,-3) -- (8,7);
        \draw [red,very thick] (-1,7) -- (8,-3);
\end{tikzpicture}
\end{document}
