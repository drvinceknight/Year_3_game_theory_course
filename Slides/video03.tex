...
\begin{center}
\begin{tikzpicture}
\node at (0,0) { $\begin{pmatrix}(.5,.5)&(0,0)\\ (.1,0)&(1,1)\end{pmatrix}$};

\pause
\node (A) at (2,2) [color=red] {Xbox};
\draw (A) edge [color=red, out=90, in=90, ->, very thick] (-.6,.5) ;
\pause

\node (B) at (3,1) [color=blue] {PS4};
\draw (B) edge [color=blue, out=-90, in=90, ->, very thick] (.8,.5) ;
\pause

\node (D) at (-3,-2) [color=red] {Xbox};
\draw (D) edge [color=red, out=90, in=180, ->, very thick] (-1.5,.25) ;
\pause

\node (C) at (-2,-3) [color=blue] {PS4};
...
u_{2}(\sigma_1,\sigma_2)&=\sum_{r\in S_1,s\in S_2}\sigma_1(r)\sigma_2(s)u_{2}(r,s)\\
...
\frame{
\begin{center}
What happens when I `always' buy an Xbox?
\end{center}
}
